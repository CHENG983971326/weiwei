...
function load() {
    console.log("123");
    $('#orderItemTable').bootstrapTable('destroy');
    $('#orderItemTable')
        .bootstrapTable(
            {
...
function reLoad() {
    $('#orderItemTable').bootstrapTable('refresh');
}
//删除订单详情
delete1=function(idd){
    var a=confirm("确定要删除该订单详情吗？");
    if (a===true) {
        $.ajax({
            type: 'post',//提交方式 post 或者get
            url: '/manage/orderitem/delete.do',//提交到那里后他的服务
            data:{"id":idd
            },
            dataType:'json',
            success: function (data) {
                if (data.code == 200) {
                    window.location.href = '/manage/orderitem/list';
                    alert("删除成功");
                }
                if (data.code == 1) {
                    window.location.href = '/manage/orderitem/list';
